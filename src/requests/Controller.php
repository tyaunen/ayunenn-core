...
abstract class Controller
{
     ...
     * @return string 通常時はリダイレクトされて処理を終了するため何も怒らない、テスト時はリダイレクト先のパスを返す
     */
    protected function redirect(string $path, array $parameter = []): void
    {
        $top_dir = rtrim(Config::getAppSetting('PATH_ROOT'), '/');
        $url = "{$top_dir}{$path}";
    ...
    public function run(): void
    {
        ...
        if (count($validate_errors) > 0) {
            foreach ($validate_errors as $error_text) {
